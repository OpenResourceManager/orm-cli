...
class BuildingStoreCommand extends APICommand
{
    ...
    protected $signature = 'building:store
                            ...
                            {--latitude= : The geo latitude of the building. }
                            {--longitude= : The geo longitude of the building. }
                            ';
    ...
    public function handle(): void
    {
        ...
        $lat = floatval($this->option('latitude'));
        $long = floatval($this->option('longitude'));
        if (empty($campusCode) && empty($campusID)) {
            $this->error('Provide a campus-id or campus-code');
            die();
        }
        $client = new BuildingClient($this->orm);
        $response = $client->store($code, $label, $lat, $long, $campusID, $campusCode);
